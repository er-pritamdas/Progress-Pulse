  ...
  return (
    <>
        ...
        <div className="p-4 sm:p-5 border-b border-base-content/8 dark:border-base-content/8 bg-base-100 flex flex-col gap-2.5">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex items-center gap-2.5 min-w-0 flex-1">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-center gap-1 sm:gap-1.5 text-base-content/60 mt-1 flex-nowrap whitespace-nowrap overflow-x-auto [scrollbar-width:none] font-medium text-[10px]">
                  ...
                  <span className="shrink-0">{fd.tenureText || calculations.tenureText}</span>
                  {fd.fdNumber && (
                    <>
                      <span className="text-base-content/30 text-[9px] shrink-0">•</span>
            ...
            <div className="flex items-center gap-1 shrink-0 opacity-80 group-hover/card:opacity-100 transition-opacity">
              <button
                type="button"
                onClick={() => onOpenInfo && onOpenInfo(fd)}
                className="p-1.5 text-info bg-info/10 hover:bg-info/20 rounded-xl transition-colors cursor-pointer"
                title="Fixed Deposit Insights & Timeline"
              >
                <Info size={14} />
              </button>
              <button
                type="button"
                onClick={() => onOpenWithdraw && onOpenWithdraw(fd)}
                className={`p-1.5 rounded-xl transition-colors cursor-pointer ${
                  isWithdrawn
                    ? "text-amber-600 dark:text-amber-400 bg-amber-500/10 hover:bg-amber-500/20"
                    : "text-primary bg-primary/10 hover:bg-primary/20"
                }`}
                title={isWithdrawn ? "Edit Withdrawal Settlement" : "Withdraw / Settle FD"}
              >
                <Coins size={14} />
              </button>
              <button
            ...
            {!isWithdrawn ? (
              <div className="flex-1 flex flex-col justify-center items-center text-center py-2 space-y-2.5">
                ...
                <button
                  ...
                  className="btn btn-sm bg-primary/15 hover:bg-primary/25 text-primary border border-primary/20 font-black rounded-2xl w-full gap-2 transition-all cursor-pointer shadow-xs"
                >
                  <Coins size={15} />
      ...
      <div className="flex md:hidden bg-base-100 rounded-2xl border border-base-content/8 dark:border-base-content/8 shadow-xs hover:shadow-md flex-col justify-between overflow-hidden transition-all duration-200">
        ...
        {/* Card Header: ONLY FD Details (No Action Icons) */}
        <div className="p-3 border-b border-base-content/8 dark:border-base-content/8 bg-base-200/30 space-y-1.5">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 min-w-0 flex-1">
              ...
              <CompanyLogo name={fd.bankName} size="w-6 h-6" type="bank" />
        ...
        <div className="p-3 space-y-2">
          {activeFace === 0 ? (
            ...
            <div className="space-y-2 animate-in fade-in duration-150">
              {/* Hero Stat: Principal Invested + Expected Maturity & Total Interest */}
              ...
              <div className="grid grid-cols-2 gap-1.5 text-xs">
                ...
                <div className="p-2 bg-base-200/50 border border-base-content/6 dark:border-base-content/6 rounded-xl space-y-0.5">
                  ...
                  <div className="font-extrabold text-xs text-base-content truncate">
                    {fd.tenureText || calculations.tenureText}
                  </div>
                  <div className="text-[9px] font-mono text-base-content/60">
                ...
                <>
                  ...
                  <div className="p-2.5 bg-amber-500/10 border border-amber-500/20 rounded-xl flex items-center justify-between gap-2">
                    ...
                    <div className="text-right text-[10px] font-mono space-y-0.5">
                      <div>
                        ...
                        <span className={`font-bold ${Number(fd.realizedGain || 0) >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600"}`}>
                          {hideNumbers ? "••••" : `${Number(fd.realizedGain || 0) >= 0 ? "+" : ""}₹${Number(fd.realizedGain || 0).toLocaleString("en-IN")}`}
                        </span>
                      </div>
                      <div>
                        <span className={`badge badge-xs font-mono font-bold border-0 ${
                          Number(fd.realizedGain || 0) >= 0
                            ? "bg-emerald-500/20 text-emerald-700 dark:text-emerald-300"
                            : "bg-rose-500/20 text-rose-700"
                        }`}>
                          {Number(fd.realizedReturnPercent || 0) >= 0 ? `+${fd.realizedReturnPercent}%` : `${fd.realizedReturnPercent}%`}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-1.5 text-xs">
                    ...
                    <div className="p-2 bg-base-200/50 border border-base-content/6 dark:border-base-content/6 rounded-xl space-y-0.5">
                      ...
                      <div className="text-[9px] font-mono text-base-content/60">
                        {Math.max(0, dayjs(fd.withdrawalDate).diff(start, "day"))} days held
                      </div>
                    </div>
                    ...
                    <div className="p-2 bg-base-200/50 border border-base-content/6 dark:border-base-content/6 rounded-xl space-y-0.5">
                      ...
                      <div className="font-extrabold text-xs font-mono text-emerald-600 dark:text-emerald-400">
                        {hideNumbers ? "••••" : `+₹${Number(fd.realizedInterest || fd.realizedGain || 0).toLocaleString("en-IN")}`}
                      </div>
                      <div className="text-[9px] font-mono text-base-content/60">
                        Total earnings
                      </div>
                    ...
                    <div className="p-2 bg-base-200/50 border border-base-content/6 dark:border-base-content/6 rounded-xl space-y-0.5">
                      ...
                      <div className="font-extrabold text-xs font-mono text-base-content">
                        {hideNumbers ? "•••" : Number(fd.penalty || 0) > 0 ? `₹${Number(fd.penalty).toLocaleString("en-IN")}` : "₹0 (None)"}
                      </div>
                      <div className="text-[9px] font-mono text-base-content/60">
                        Premature charges
                      </div>
                  ...
                  <div className="px-2.5 py-1.5 bg-base-200/30 border border-base-content/6 dark:border-base-content/6 rounded-lg text-[10px] flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => onRemoveWithdrawal ? onRemoveWithdrawal(fd) : onOpenWithdraw && onOpenWithdraw(fd)}
