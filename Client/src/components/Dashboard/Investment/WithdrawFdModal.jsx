...
export default function WithdrawFdModal({
  isOpen,
  onClose,
  onSaveSettlement,
  fd = null,
  ...
  const [withdrawalDate, setWithdrawalDate] = useState(
    fd?.withdrawalDate || dayjs().format("YYYY-MM-DD")
  );
  const [amountGotInput, setAmountGotInput] = useState(
    String(fd?.totalPayout || fd?.maturityAmount || principal || "")
  );
  const [penaltyInput, setPenaltyInput] = useState(String(fd?.penalty || "0"));
  ...
  useEffect(() => {
    if (fd) {
      setWithdrawalDate(
        fd.withdrawalDate
          ? dayjs(fd.withdrawalDate).format("YYYY-MM-DD")
          : dayjs().format("YYYY-MM-DD")
      );
      const defaultGot =
        fd.totalPayout !== undefined && fd.totalPayout !== null && fd.totalPayout > 0
          ? fd.totalPayout
          : (fd.maturityAmount || principal || "");
      setAmountGotInput(String(defaultGot));
      setPenaltyInput(String(fd.penalty ?? "0"));
      setErrorMsg("");
    }
  }, [isOpen, fd, principal]);
  ...
  const start = dayjs(fd?.startDate || new Date());
  ...
  const handleResetToActive = async () => {
    if (!window.confirm("Are you sure you want to remove this Withdrawn transaction and restore this Fixed Deposit to Active?")) return;
    setIsSubmitting(true);
    try {
      const resetPayload = {
        ...fd,
